...
void display(void)
{
    ...
    for (int i = 0; i < shapes.size(); i++)
    {
        ...
        for (int j = i+1; j < shapes.size(); j++)
        {
            ...
            {
                Collide(i, j, shapes);
                double r = shapes[i]->getred();
                double g = shapes[i]->getgreen();
                double b = shapes[i]->getblue();
                double r1 = shapes[j]->getred();
                double g1 = shapes[j]->getblue();
                double b1 = shapes[j]->getgreen();
                shapes[j]->Mgreen = g;
                shapes[j]->Mred = r;
                shapes[j]->Mblue = b;
                shapes[i]->Mgreen = g1;
