...
bool v2 = false;
bool solid=true;
const double COLLISION_FRICTION = .95;
...
void display(void)
{
    ...
    for (int i = 0; i < shapes.size() - 1; i++)
    {
        ...
        double inextX = shapes[i]->getnextx();
        double inextY = shapes[i]->getnexty();
        double ired = shapes[i]->getr();
        double igreen = shapes[i]->getg();
        double iblue = shapes[i]->getb();
        ...
        for (int j = i+1; j < shapes.size(); j++)
        {
            ...
            double jred = shapes[j]->getr();
            double jgreen = shapes[j]->getg();
            double jblue = shapes[j]->getb();
            double jradius = shapes[j]->getradius();
            double a = pow((jnextx-inextX),2);
            double b = pow((jnexty-inextY),2);
            double distance = sqrt(a+b);
            if (v2) {
                if (distance < radius+jradius && ired == jred && igreen == jgreen && iblue == jblue)
                {
                    Collide(i, j, shapes);
...
void mouse(int mouse_button, int state, int x, int y)
{
  ...
  if (mouse_button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) 
    {
        shapes.push_back(new Circle(x, y, x+50, y, 0, 0, 1, solid));
    }
